...
In sub_428990 function, it reads in a user-provided parameter `mac` and passes its value to Uci_Add_List function by strcat ,which is defined in libcscommon.so.

However ,the value of the `mac`  is inserted into `v11`  using `snprintf`,and the value of v11 will be handled by the function CsteSystem.
...
We set `mac` as **\`ls>./setAccessDeviceCfg.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setAccessDeviceCfg","mac":"`ls>./setAccessDeviceCfg.txt`"
}
```
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setAccessDeviceCfg.txt` was executed successfully.
