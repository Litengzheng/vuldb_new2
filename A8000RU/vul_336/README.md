...
In sub_430128 function, it reads in a user-provided parameter `provider` and passes its value to Uci_Set_Str_By_Idx function,which is defined in libcscommon.so.

As we can see ,the parameter `v3` is  passed to  Uci_Set_Str  function.  

Then the value of the `provider`  is  inserted into `v11`  using `snprintf`,and the value of v11 will be handled by the function CsteSystem.
...
We set `provider` as **\`ls>./setDdnsCfg.txt\`** , and the router will execute it,such as:
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setDdnsCfg.txt` was executed successfully.
