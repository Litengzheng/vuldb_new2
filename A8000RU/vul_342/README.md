...
In sub_4311C8 function, it reads in a user-provided parameter `FileName` .

However ,the value of the `FileName`  is inserted into `stat_buf`  using `snprintf`,and the value of v11 will be handled by the function CsteSystem.
...
We set `FileName` as **\`ls>./UploadOpenVpnCert.txt\`** , and the router will execute it,such as:
...
{"topicurl":"UploadOpenVpnCert","FileName":"`ls>./UploadOpenVpnCert.txt`"
}
```
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./UploadOpenVpnCert.txt` was executed successfully.
