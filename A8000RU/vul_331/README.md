...
In sub_423CC8 function, it reads in a user-provided parameter `ip` .

<div  align="center"><img src="./img/function.png" style="zoom:80%;" /></div>

However ,the value of the `ip`  is inserted into `v6`  using `snprintf`,and the value of v6 will be handled by the function CsteSystem.
...
We set `ip` as **\`ls>./setDiagnosisCfg.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setDiagnosisCfg","ip":"`ls>./setDiagnosisCfg.txt`"
}
```
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setDiagnosisCfg.txt` was executed successfully.
