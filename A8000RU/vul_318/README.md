...
In  sub_41F610 function, it reads in a user-provided parameter `tty_server` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.

However ,the value of the `tty_server`  is inserted into `v11`  using `snprintf`,and the value of v11 will be handled by the function CsteSystem.
...
We set `tty_server` as **\`ls>./setAdvancedInfoShow.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setAdvancedInfoShow","tty_server":"`ls>./setAdvancedInfoShow.txt`"
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setAdvancedInfoShow.txt` was executed successfully.
