...
In sub_420A88 function, it reads in a user-provided parameter `enable` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.
...
We set `enable` as **\`ls>./setQosCfg.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setQosCfg","enable":"`ls>./setQosCfg.txt`"
}
```
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setQosCfg.txt` was executed successfully.
