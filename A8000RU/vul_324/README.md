...
In sub_42A3D8 function, it reads in a user-provided parameter `user` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.
...
We set `user` as **\`ls>./setVpnAccountCfg.txt\`** , and the router will execute it,such as:
...
{"topicurl":"setVpnAccountCfg","user":"`ls>./setVpnAccountCfg.txt`"
}
```
...
After submitting the HTTP request, we observed that the txt file was successfully created, and its content was precisely the list of filenames from the folder. This confirms that the command `ls>./setVpnAccountCfg.txt` was executed successfully.
